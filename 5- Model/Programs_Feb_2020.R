...
cmnpe <- function(data_w_out, DF_P, DF_R, B.knots, q.order = 2, cov_data=matrix(0,1,1), Pcov_data=NULL, cov_mat="CS",plots=TRUE,boot=FALSE,B=1000,seed.val = 234,TRANS=FALSE,full_sig=FALSE,zero_covs=NULL,slope=FALSE){
...
model_fit <- function(data_w_out,all_data,DF_P,DF_R,B.knots,q.order,cov_data=0,Pcov_data,symm=FALSE,diag=FALSE,plots=TRUE,boot=FALSE,B=2000,seed.val=234,TRANS=FALSE,full_sig=FALSE){
  ...
  Boot_data <- NULL
  if(!boot){
    resid <- pred_results$resid
    mis_ordered <- cbind(pred_results$newdata,resid)
    ...
    plot_data <- data.frame(Y = ordered$Y,SE_var = ordered$SE_var,country=ordered$country,year=ordered$year,pred=pred_new,lower_CI=lower_CI,upper_CI=upper_CI,lower_CI2=lower_CI2,upper_CI2=upper_CI2,sigma_T_est=sigma_T_est,sigma_Y_est=sigma_Y_est,resid = ordered$resid,pred_fixed=pred_fixed,pred_fixpen=pred_fixpen)
...
prediction <- function(result,newdata,plots=TRUE,boot){
  ...
  resid = (data_frame$Y-pred_mat$mu_T)/sqrt(diag(pred_mat$sigma_Y)) 
  ...
  resid_final <- cov_w_preds$Y
  resid_final[!is.na(resid_final)] <- resid
  Ret_list <- list(newdata = cov_w_preds, pred_orig=pred_mat$mu_T, sigma_orig = pred_mat$sigma_T,b=b,D=pred_mat$D,D_new=D_new,X_new=X_all,Z_new=Z_all,X_orig=X_orig,Z_orig=Z_orig,COV_beta_b=COV_beta_b,SIGMA_T=SIGMA_T,SIGMA_Y=SIGMA_Y,df=df,resid = resid_final)
  return(Ret_list)
}
...
output_function <- function(Estimation, all_data, data_one, marker, date,path){
  
  
  #View(Estimation$plot_data %>% filter(country == "Bangladesh"))
  ...
  adj_cov <- cov_data[,which(colnames(cov_data) %in% zero_covs)]
  
  plot_data <- as_tibble(Estimation$plot_data) %>% 
    bind_cols(Sex = all_data$Sex) %>% 
    mutate(
      val = marker) 
  
  #View(plot_data %>% filter(country == "Bangladesh" & year == 2011))
  
    plot_data <- plot_data %>% 
      bind_cols(
        all_Y = all_data$Y_all, 
        all_SE_var = all_data$SE_var) %>% 
      mutate(
        all_SE_var = all_SE_var*((1/all_Y) + 1/(1-all_Y)), 
        all_Y = log(all_Y/(1-all_Y))
      ) %>% 
      mutate(
        SE_var = case_when(
          SE_var == 0 ~ all_SE_var,
      ...
      select(-c(all_SE_var,all_Y))
    
  
  if(length(smart_cov) == 1){
    plot_data$adj_pred_ns <- plot_data$Y_NS - adj_cov*smart_cov
    plot_data$adj_pred <- plot_data$Y - adj_cov*smart_cov
  }else{
    plot_data$adj_pred_ns <- c(plot_data$Y_NS - adj_cov%*%smart_cov) 
    plot_data$adj_pred <- c(plot_data$Y - adj_cov%*%smart_cov ) 
  }
  ...
  plot_data <- plot_data %>% 
    ...
    select(
      -c("Y","pred","lower_CI2","upper_CI2","sigma_T_est","sigma_Y_est","SE_var","Y_NS")
    ) %>% 
    select(
      c(val,country, year, Sex, Point.Estimate, Std.Err, 
        Prediction, SE_mean_pred, SE_pred, lower_CI,  
        upper_CI),everything()
    )
  #plot_data
  ...
  PP_plot_data <- data_one %>% 
    select(-c("country", "year", "Sex")) %>% 
    rename(SE_pred.y = SE_pred) %>% 
    bind_cols(plot_data) %>% 
    mutate(
    ...
    select(-c(SE_Imput_Ind, SE_pred.y)) %>% 
